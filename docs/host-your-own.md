...
This can be useful if you don't want to trust the client files that are hosted on another instance but still want to connect to devices that use https://drop.erikraft.com.

### Specify Signaling Server

```bash
SIGNALING_SERVER="drop.erikraft.com"
```
...
DONATION_BUTTON_LINK="https://ko-fi.com/erikraft"
DONATION_BUTTON_TITLE="ko-fi.com/erikraft"
...
server {
    ...
    ssl_certificate /etc/ssl/certs/erikraftdrop-dev.crt;
    ssl_certificate_key /etc/ssl/certs/erikraftdrop-dev.key;
...
```bash
git clone https://github.com/erikraft/Drop.git && erikraftdrop Drop
```
```bash
...
- To debug the Node.js server, run `docker logs erikraftdrop`.
