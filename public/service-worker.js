...
const relativePathsToCache = [
    './', 'index.html', 'manifest.json', 'ads.html',
    'styles/styles-main.css', 'styles/styles-deferred.css', 'styles/content-moderation.css',
    'scripts/browser-tabs-connector.js', 'scripts/localization.js', 'scripts/main.js', 'scripts/network.js',
    'scripts/persistent-storage.js', 'scripts/ui.js', 'scripts/ui-main.js', 'scripts/util.js',
    'scripts/content-moderation.js', 'scripts/pairdrop-adapter.js', 'scripts/erikraft-qr.js',
    'scripts/security-layer.js', 'scripts/animated-qr-controls.js', 'scripts/animated-qr-file-size.js',
    'scripts/animated-qr-screen-awake.js', 'scripts/worker/canvas-worker.js', 'scripts/libs/heic2any.min.js',
    'scripts/libs/jsQR.js', 'scripts/libs/no-sleep.min.js', 'scripts/libs/qr-code-styling.js', 'scripts/qr-helper.js',
    'scripts/libs/zip.min.js', 'sounds/blop.mp3', 'sounds/blop.ogg', 'sounds/ads.mp3',
    'images/favicon-96x96.png', 'images/favicon-96x96-notification.png', 'images/android-chrome-192x192.png',
    'images/android-chrome-192x192-maskable.png', 'images/android-chrome-512x512.png',
    'images/android-chrome-512x512-maskable.png', 'images/apple-touch-icon.png', 'images/badges/BadgeLinux.png',
    'images/badges/F-Droid-get-it-on.png', 'images/badges/Get-it-from-Opera-Addons.png',
    'images/badges/Get_it_on_APKPure_English.png', 'images/badges/Get_it_on_Google_Play_Store_English.png',
    'images/badges/Get_it_on_Shortcuts_English.png', 'images/badges/Get_the_add-on_Thunderbird.png',
    'images/badges/Support_me_on_Ko-fi.png', 'images/badges/Get it on WEB.png',
    'fonts/OpenSans/static/OpenSans-Medium.ttf',
    'lang/ar.json', 'lang/be.json', 'lang/bg.json', 'lang/ca.json', 'lang/cs.json', 'lang/da.json',
    'lang/de.json', 'lang/en.json', 'lang/es.json', 'lang/et.json', 'lang/eu.json', 'lang/fa.json',
    'lang/fr.json', 'lang/he.json', 'lang/hu.json', 'lang/id.json', 'lang/it.json', 'lang/ja.json',
    'lang/kn.json', 'lang/ko.json', 'lang/nb.json', 'lang/nl.json', 'lang/nn.json', 'lang/pl.json',
    'lang/pt-BR.json', 'lang/ro.json', 'lang/ru.json', 'lang/sk.json', 'lang/ta.json', 'lang/tr.json',
    'lang/uk.json', 'lang/zh-CN.json', 'lang/zh-HK.json', 'lang/zh-TW.json'
];
...
const cacheResponseIfValid = async (request, response) => {
    ...
    } catch (error) { console.warn('[SW] Could not update cache for:', request.url, error); }
};
...
const fetchCritical = async request => {
    try { return await fetchNetwork(request); }
    catch (error) {
    ...
    event.waitUntil((async () => {
        ...
        const results = await Promise.allSettled(relativePathsToCache.map(async path => {
            try {
                ...
                if (!response.ok || response.type === 'opaque') throw new Error(`HTTP ${response.status} for ${path}`);
                await cache.put(new Request(new URL(path, rootUrl).href), response);
                return path;
            } catch (error) { console.warn('[SW] Optional pre-cache failed:', path, error); return null; }
        }));
...
self.addEventListener('activate', event => {
    ...
    event.waitUntil((async () => {
        ...
        await Promise.all(cacheNames.map(cacheName => {
            if (cacheName.startsWith('erikraftdrop-cache-') && cacheName !== cacheTitle) return caches.delete(cacheName);
...
self.addEventListener('fetch', event => {
    ...
    if (request.method !== 'GET' || !isSameOrigin(request)) {
        if (request.method === 'POST' && isSameOrigin(request)) {
            ...
            event.respondWith((async () => { const shareUrl = await evaluateRequestData(request); return Response.redirect(encodeURI(shareUrl), 302); })());
        } else if (!isSameOrigin(request)) event.respondWith(fetch(request));
        return;
    }
    if (doNotCacheRequest(request)) { event.respondWith(fetch(request, {cache: 'no-store'})); return; }
...
const evaluateRequestData = async request => {
    try {
        ...
        if (files && files.length > 0) {
            const fileObjects = [];
            for (let i = 0; i < files.length; i++) fileObjects.push({name: files[i].name, buffer: await files[i].arrayBuffer()});
            ...
            await Promise.all(fileObjects.map(fileObject => new Promise((resolve, reject) => {
                try {
                    ...
                    const objectStoreRequest = transaction.objectStore('share_target_files').add(fileObject);
