(function setupAnimatedQRRuntimeControls() {
    ...
    const install = tx => {
        ...
        tx.initialize = function () {
            ...
            const view = document.getElementById('qr-send-active-view'); if (view) view.hidden = false;
            render(this); sync(this);
        };
        ...
        tx.resume = function () {
            ...
            const tick = () => {
                if (!this.running || this.paused || !this.initialized || !this.frames?.length) { stopTimer(this); sync(this); return; }
                render(this); this.currentIndex = (this.currentIndex + 1) % this.frames.length; sync(this);
    ...
    const makeButton = (id, text, label, handler, template) => {
        ...
        button = template.cloneNode(true); button.id = id; button.type = 'button'; button.removeAttribute('data-i18n-key'); button.textContent = text; button.setAttribute('aria-label', label); button.title = label; button.onclick = handler; button.hidden = false;
        return button;
    };
    const setup = tx => {
        const buttons = document.getElementById(IDS.activeButtons), pause = document.getElementById(IDS.pause);
        ...
        pause.onclick = event => { event.preventDefault(); event.stopPropagation(); const t = getTransmitter(); if (!t?.initialized) return; t.paused ? t.resume() : t.pause(); sync(t); };
        if (!document.getElementById(IDS.seek)) {
            const wrapper = document.createElement('div'); wrapper.id = IDS.seekWrapper; wrapper.className = 'fw column gap-1'; wrapper.style.cssText = 'width:100%;margin:8px 0 0;';
            const seek = document.createElement('input'); seek.type = 'range'; seek.id = IDS.seek; seek.className = 'fw'; seek.style.cssText = 'width:100%;accent-color:#0d6efd;cursor:pointer;touch-action:pan-x;'; seek.setAttribute('aria-label', 'Posição do QR animado'); seek.addEventListener('input', e => getTransmitter()?.seekFrame?.(Number(e.target.value))); wrapper.appendChild(seek); buttons.parentNode.insertBefore(wrapper, buttons);
        }
        if (!document.getElementById(IDS.frameInput)) {
            const group = document.createElement('div'); group.id = IDS.frameGroup; group.className = 'row center gap-2'; group.style.cssText = 'width:100%;justify-content:center;margin:8px 0 0;flex-wrap:wrap;';
            const input = document.createElement('input'); input.type = 'number'; input.id = IDS.frameInput; input.className = 'btn btn-rounded btn-grey'; input.min = '1'; input.step = '1'; input.inputMode = 'numeric'; input.style.cssText = 'width:min(90px,24vw);text-align:center;'; input.setAttribute('aria-label', 'Número do QR');
            ...
            const goFrame = () => { const value = Number.parseInt(input.value, 10); if (Number.isFinite(value)) getTransmitter()?.seekFrame?.(value - 1); }; go.onclick = goFrame; input.addEventListener('keydown', e => { if (e.key === 'Enter') { e.preventDefault(); goFrame(); } }); group.append(input, go); buttons.parentNode.insertBefore(group, buttons);
        }
        sync(tx); return true;
    };
